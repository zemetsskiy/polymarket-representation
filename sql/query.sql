...
user_trades AS (
    SELECT
        ...
        timestamp,
        -- Determine user_id based on the array join index (1 for maker, 2 for taker)
        if(idx = 1, maker, taker) AS user_id,
        if(idx = 1, taker, maker) AS agent,
        
        -- Determine amounts based on the array join index
        -- idx 1 (Maker): 
        --   if maker_asset_id='0' (Buy Token): usdc=maker_amount, token=taker_amount
        --   if taker_asset_id='0' (Sell Token): usdc=taker_amount, token=maker_amount
        if(idx = 1, 
        ...
        -- Pre-calculate token_id to enable Equi-Join later
        if(asset_in = '0', asset_out, asset_in) AS token_id
        
    FROM polymarket.orders
    -- Generate 2 rows for each order: 1 for Maker, 2 for Taker
...
market_operations AS (
    ...
    INNER JOIN market_tokens mt 
        ON ut.token_id = mt.token_id -- Optimized Equi-Join
),
...
SELECT 
    ...
    AVG(operations_count) as avg_operations_count,
    SUM(absolute_profit) / 1e6 as absolut_profit,
    AVG(relative_profit) as AP,
    SUM(total_gained) / 1e6 as total_gained_,
    SUM(total_spent) / 1e6 as total_spent_,
    if(total_spent_ = 0, 0, total_gained_ / total_spent_) as WAP,
    MIN(day_enter) as day_enter,
    MAX(day_exit) as day_exit,
    pow(AP, 365 / dateDiff('day', day_enter, day_exit)) as annual_ap,
    pow(WAP, 365 / dateDiff('day', day_enter, day_exit)) as annual_wap,
    -- day_exit >= '2025-03-01' as active -- hardcoded date commented out; consider using a dynamic date or parameter
    1 as active -- Placeholder for active status since date check is removed
FROM user_profits_by_tokens
GROUP BY user_id
)
select * FROM user_grouped limit 10